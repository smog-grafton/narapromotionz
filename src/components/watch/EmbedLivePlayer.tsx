...
export function EmbedLivePlayer({ src, title, chatOpen, onToggleChat }: Props) {
  ...
  }, [chatOpen, controlsVisible, playing]);
  ...
  return (
    ...
    >
      ...
      <div className={`pointer-events-none absolute inset-x-0 top-0 z-20 h-24 bg-gradient-to-b from-black/85 to-transparent transition-opacity duration-300 ${controlsVisible ? "opacity-100" : "opacity-0"}`} />
      <div className={`pointer-events-none absolute inset-x-0 bottom-0 z-20 h-24 bg-gradient-to-t from-black/85 to-transparent transition-opacity duration-300 ${controlsVisible ? "opacity-100" : "opacity-0"}`} />
      <div className={`pointer-events-none absolute left-3 top-3 z-30 flex items-center gap-2 border border-[#e1252b]/60 bg-black/85 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.18em] text-white transition-opacity duration-300 sm:left-5 sm:top-5 ${controlsVisible ? "opacity-100" : "opacity-0"}`}>
      ...
      <div className={`absolute bottom-3 left-3 right-3 z-30 flex items-center justify-between gap-3 transition-opacity duration-300 sm:bottom-5 sm:left-5 sm:right-5 ${controlsVisible ? "opacity-100" : "pointer-events-none opacity-0"}`}>
